...
class VDUHelper:
   # ###### Constructor #####################################################
   def __init__(self, context, testMode = False):
      # ====== Initialise object ============================================
      ...
      if self.testMode == False:
         ...
         #self.sshproxy_module = SSHProxyCharm
   ...
   def execute(self, commands):
      ...
      if self.testMode == False:
         ...
         proxy = self.context.get_ssh_proxy()
         stdout, stderr = proxy.run(commands)
...
"""
   ...
   def configureGit(self, name, email):
      ...
      self.createFileFromString('/home/ubuntu/.gitconfig', """\
[user]
        name = {name}
        email = {email}
[push]
...
""".format(name = name, email = email))
      self.runInShell('sudo chown ubuntu:ubuntu /home/ubuntu/.gitconfig')
      self.endBlock()
